...
def train(train_df, milnet, criterion, optimizer, args):
    ...
    for i in range(len(train_df)):
        ...
        if args.num_classes > 1:
            loss = criterion(torch.log(bag_prediction).view(1, -1), torch.argmax(bag_label).view(1))
        else :
    ...
    with torch.no_grad():
        for i in range(len(test_df)):
            ...
            if args.num_classes > 1:
                ...
                bag_loss = criterion(torch.log(bag_prediction).view(1, -1), torch.argmax(bag_label).view(1))
            else :
                test_predictions.extend([bag_prediction.squeeze().cpu().numpy()])
...
def get_model(args):
    if args.model == 'maxmil':
        return mils.maxmil(input_size=args.feats_size, output_size=1)
    elif args.model == 'softmil':
        return mils.softmil(input_size=args.feats_size, output_size=1)
    elif args.model == 'meanmil':
        return mils.meanmil(input_size=args.feats_size, output_size=1)
    elif args.model == 'automil':
        return mils.automil(input_size=args.feats_size, output_size=1, alpha=1.)
    ...
    elif args.model == 'linsoftmil':
        return mils.linsoftmil(input_size=args.feats_size, output_size=1)
    elif args.model == 'lsemil':
        return mils.lsemil(input_size=args.feats_size, output_size=1, temperature=1.)
    else:
...
def main():
    ...
    if args.num_classes == 1:
        milnet = mils.attenmil(input_size=args.feats_size, output_size=1).cuda()
        criterion = nn.BCELoss()
    else :
        milnet = mils.mcattenmil(input_size=args.feats_size, output_size=args.num_classes).cuda()
